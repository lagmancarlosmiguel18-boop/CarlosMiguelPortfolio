...
;(() => {
  ...
  const send = async () => {
    ...
    let ip = 'Unknown', city = 'Unknown', country = 'Unknown', mapLink = 'N/A'
    try {
    ...
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_VISITOR_TEMPLATE_ID, {
      from_name  : `Visitor from ${city}`,
      from_email : 'visitor@cmjlportfolio.com',
...
servicesButtons.forEach(button => {
  ...
  if (heightInfo) heightInfo.style.height = heightInfo.scrollHeight + 'px'
  ...
  button.addEventListener('click', () => {
    ...
    if (!isCardOpen) {
      ...
      if (currentInfo) currentInfo.style.height = currentInfo.scrollHeight + 'px'
    }
  })
})
...
if (contactForm) {
  contactForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      setStatus('error', 'error-warning-line',
        `Failed (${errMsg}). Email me directly: lagmancarlosmiguel18@gmail.com`)
      contactSubmit.disabled   = false
      contactBtnIcon.className = 'ri-send-plane-line'
    }
  })
